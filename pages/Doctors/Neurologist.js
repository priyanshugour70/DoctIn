import React from 'react'

const Neurologist = () => {
  return (
    <div>
      <section className="flex items-center py-20 bg-gray-100 font-poppins dark:bg-gray-800 ">
  <div className="justify-center flex-1 max-w-6xl px-4 py-4 mx-auto lg:py-6 md:px-6">
    <div className="flex flex-wrap items-center">
      <div className="w-full px-4 mb-10 xl:w-1/2 lg:mb-8">
        <div className="flex flex-wrap">
          <div className="w-full px-4 md:w-1/2">
            <img
              src="https://images.pexels.com/photos/8260442/pexels-photo-8260442.jpeg?auto=compress&cs=tinysrgb&w=1600"
              alt=""
              className="object-cover w-full mb-6 rounded-lg h-80"
            />
            <img
              src="https://images.pexels.com/photos/3259629/pexels-photo-3259629.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="object-cover w-full rounded-lg h-80"
            />
          </div>
          <div className="w-full px-4 md:w-1/2 xl:mt-11">
            <img
              src="https://images.pexels.com/photos/7583370/pexels-photo-7583370.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="object-cover w-full mb-6 rounded-lg h-80"
            />
            <img
              src="https://images.pexels.com/photos/7579825/pexels-photo-7579825.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt=""
              className="object-cover w-full rounded-lg h-80"
            />
          </div>
        </div>
      </div>
      <div className="w-full px-4 mb-10 xl:w-1/2 lg:mb-8">
        <span className="text-sm font-semibold text-blue-500 dark:text-blue-400">
          Doctor
        </span>
        <h2 className="mt-2 mb-4 text-2xl font-bold text-gray-700 dark:text-gray-300">
          Neurologist
        </h2>
        <p className="mb-4 text-base leading-7 text-gray-500 dark:text-gray-400">
          
A neurologist is a medical professional specializing in the diagnosis and treatment of diseases and disorders affecting the nervous system. They work to identify and manage conditions such as epilepsy, stroke, multiple sclerosis, and neurodegenerative diseases like Alzheimer's and Parkinson's.
        </p>
        <ul className="mb-10">
          <li className="flex items-center mb-4 text-base text-gray-600 dark:text-gray-400">
            <span className="mr-3 text-blue-500 dark:text-blue-400 ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width={16}
                height={16}
                fill="currentColor"
                className="w-6 h-6 bi bi-1-circle-fill"
                viewBox="0 0 16 16"
              >
                <path
                  fillRule="evenodd"
                  d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0ZM9.283 4.002V12H7.971V5.338h-.065L6.072 6.656V5.385l1.899-1.383h1.312Z"
                />
              </svg>
            </span>
            Highly skilled neurologist specializing in neurological disorders.
          </li>
          <li className="flex items-center mb-4 text-base text-gray-600 dark:text-gray-400">
            <span className="mr-3 text-blue-500 dark:text-blue-400">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width={16}
                height={16}
                fill="currentColor"
                className="w-6 h-6 bi bi-2-circle-fill"
                viewBox="0 0 16 16"
              >
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0ZM6.646 6.24c0-.691.493-1.306 1.336-1.306.756 0 1.313.492 1.313 1.236 0 .697-.469 1.23-.902 1.705l-2.971 3.293V12h5.344v-1.107H7.268v-.077l1.974-2.22.096-.107c.688-.763 1.287-1.428 1.287-2.43 0-1.266-1.031-2.215-2.613-2.215-1.758 0-2.637 1.19-2.637 2.402v.065h1.271v-.07Z" />
              </svg>
            </span>
            Experienced neurologist providing comprehensive care for brain conditions.
          </li>
          <li className="flex items-center mb-4 text-base text-gray-600 dark:text-gray-400">
            <span className="mr-3 text-blue-500 dark:text-blue-400">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width={16}
                height={16}
                fill="currentColor"
                className="w-6 h-6 bi bi-3-circle-fill"
                viewBox="0 0 16 16"
              >
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0Zm-8.082.414c.92 0 1.535.54 1.541 1.318.012.791-.615 1.36-1.588 1.354-.861-.006-1.482-.469-1.54-1.066H5.104c.047 1.177 1.05 2.144 2.754 2.144 1.653 0 2.954-.937 2.93-2.396-.023-1.278-1.031-1.846-1.734-1.916v-.07c.597-.1 1.505-.739 1.482-1.876-.03-1.177-1.043-2.074-2.637-2.062-1.675.006-2.59.984-2.625 2.12h1.248c.036-.556.557-1.054 1.348-1.054.785 0 1.348.486 1.348 1.195.006.715-.563 1.237-1.342 1.237h-.838v1.072h.879Z" />
              </svg>
            </span>
            Compassionate neurologist dedicated to improving patients' neurological health.
          </li>
          <li className="flex items-center mb-4 text-base text-gray-600 dark:text-gray-400">
            <span className="mr-3 text-blue-500 dark:text-blue-400 ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width={16}
                height={16}
                fill="currentColor"
                className="w-6 h-6 bi bi-4-circle-fill"
                viewBox="0 0 16 16"
              >
                <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0ZM7.519 5.057c-.886 1.418-1.772 2.838-2.542 4.265v1.12H8.85V12h1.26v-1.559h1.007V9.334H10.11V4.002H8.176c-.218.352-.438.703-.657 1.055ZM6.225 9.281v.053H8.85V5.063h-.065c-.867 1.33-1.787 2.806-2.56 4.218Z" />
              </svg>
            </span>
            Expert neurologist utilizing advanced techniques to diagnose and treat.
          </li>
        </ul>
        <span
          className="px-4 py-2 text-gray-100 bg-blue-500 rounded-md dark:bg-blue-400 dark:hover:bg-blue-500 hover:bg-blue-600"
        >
          Neuro
        </span>
      </div>
    </div>
  </div>
</section>

{/* ************************************ */}

<section className="flex items-center py-24 bg-stone-100 font-poppins dark:bg-gray-800 ">
  <div className="justify-center flex-1 max-w-6xl px-4 py-6 mx-auto lg:py-4 md:px-6">
    <div className="mb-10 text-center">
      <span className="block mb-4 text-xs font-semibold leading-4 tracking-widest text-center text-blue-500 uppercase dark:text-gray-400">
        Neurologist
      </span>
      <h1 className="text-3xl font-bold capitalize dark:text-white">
        {" "}
        Doctors{" "}
      </h1>
    </div>
    <div className="flex flex-wrap -mx-4">
      <NeurologistDoctors/>
    </div>
  </div>
</section>


    </div>
  )
}

export default Neurologist












const NeurologistDoctors = () => {
  const data = [
    {
      id: 1,
      name: 'Aarav Patel',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 2,
      name: 'Aarushi Gupta',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 3,
      name: 'Aditi Singh',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 4,
      name: 'Advaita Sharma',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 5,
      name: 'Aishani Desai',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 6,
      name: 'Akash Patel',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 7,
      name: 'Alka Singh',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 8,
      name: 'Amaira Gupta',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 9,
      name: 'Amrita Sharma',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 10,
      name: 'Ananya Desai',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 11,
      name: 'Anika Patel',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 12,
      name: 'Anjali Singh',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 13,
      name: 'Ankit Sharma',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 14,
      name: 'Anshika Gupta',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 15,
      name: 'Arnav Patel',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 16,
      name: 'Arpita Singh',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
      id: 17,
      name: 'Arsh Desai',
      ImgSrc: 'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
      desg: 'Neurologist'
    },
    {
       id:18,
       name:'Aryan Patel',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:19,
       name:'Avni Gupta',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:20,
       name:'Bhavya Sharma',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:21,
       name:'Devanshi Patel',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:22,
       name:'Dhruv Singh',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:23,
       name:'Diya Desai',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:24,
       name:'Divya Patel',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:25,
       name:'Ishaan Gupta',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:26,
       name:'Ishani Sharma',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:27,
       name:'Jhanvi Patel',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:28,
       name:'Kavya Singh',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:29,
       name:'Khushi Desai',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    },
    {
       id:30,
       name:'Manav Patel',
       ImgSrc:'https://images.pexels.com/photos/6129111/pexels-photo-6129111.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1',
       desg:'Neurologist'
    }
  ];

  return (
    <>
      {data.map((person) => (
        <div className="w-full px-4 mb-4 lg:w-1/3 sm:w-1/2 lg:mb-10" Key={person.id}>
        <div className="overflow-hidden">
          <img
            className="object-cover w-full transition-all hover:scale-110 h-72 "
            src={person.ImgSrc}
            alt=""
          />
        </div>
        <h2 className="mt-4 text-xl font-bold dark:text-gray-400">
        {person.name}
        </h2>
        <p className="mt-2 text-sm text-blue-500 dark:text-blue-400">{person.desg}</p>
        <div className="flex items-center justify-start mt-4">
          <a
            className="inline-block mr-5 text-gray-800 hover:text-blue-700 dark:text-gray-400 dark:hover:text-blue-400"
            href="#"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width={16}
              height={16}
              fill="currentColor"
              className="w-6 h-6 bi bi-facebook "
              viewBox="0 0 16 16"
            >
              <path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951z" />
            </svg>
          </a>
          <a
            className="inline-block mr-5 text-gray-800 dark:text-gray-400 hover:text-blue-400 dark:hover:text-blue-400"
            href="#"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width={16}
              height={16}
              fill="currentColor"
              className="w-6 h-6 bi bi-twitter"
              viewBox="0 0 16 16"
            >
              <path d="M5.026 15c6.038 0 9.341-5.003 9.341-9.334 0-.14 0-.282-.006-.422A6.685 6.685 0 0 0 16 3.542a6.658 6.658 0 0 1-1.889.518 3.301 3.301 0 0 0 1.447-1.817 6.533 6.533 0 0 1-2.087.793A3.286 3.286 0 0 0 7.875 6.03a9.325 9.325 0 0 1-6.767-3.429 3.289 3.289 0 0 0 1.018 4.382A3.323 3.323 0 0 1 .64 6.575v.045a3.288 3.288 0 0 0 2.632 3.218 3.203 3.203 0 0 1-.865.115 3.23 3.23 0 0 1-.614-.057 3.283 3.283 0 0 0 3.067 2.277A6.588 6.588 0 0 1 .78 13.58a6.32 6.32 0 0 1-.78-.045A9.344 9.344 0 0 0 5.026 15z" />
            </svg>
          </a>
          <a
            className="inline-block text-gray-800 dark:text-gray-400 hover:text-pink-400 dark:hover:text-pink-400 "
            href="#"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width={16}
              height={16}
              fill="currentColor"
              className="w-6 h-6 bi bi-instagram"
              viewBox="0 0 16 16"
            >
              <path d="M8 0C5.829 0 5.556.01 4.703.048 3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297.04.852.174 1.433.372 1.942.205.526.478.972.923 1.417.444.445.89.719 1.416.923.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046.78.035 1.204.166 1.486.275.373.145.64.319.92.599.28.28.453.546.598.92.11.281.24.705.275 1.485.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598-.28.11-.704.24-1.485.276-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598 2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485-.038-.843-.046-1.096-.046-3.233 0-2.136.008-2.388.046-3.231.036-.78.166-1.204.276-1.486.145-.373.319-.64.599-.92.28-.28.546-.453.92-.598.282-.11.705-.24 1.485-.276.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217 4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334 2.667 2.667 0 0 1 0-5.334z" />
            </svg>
          </a>
        </div>
      </div>
      ))}
      </>
  );
}